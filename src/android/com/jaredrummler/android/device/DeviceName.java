...
public class DeviceName extends CordovaPlugin {
	...
	private static final String TAG = "CORDOVA-PLUGIN-PIM";
		...
		if(action.equals("getInfo")){
			...
			DeviceName.with(context).request(new DeviceName.Callback() {
				@Override public void onFinished(DeviceName.DeviceInfo info, Exception error) {
					...
					try{
						Log.d(TAG, "Begin onFinished");
						JSONObject jObj = new JSONObject();
						jObj.put("manufacturer", info.manufacturer);
						jObj.put("marketName", info.marketName);
						jObj.put("model", info.model);
						jObj.put("codename", info.codename);
						jObj.put("deviceName", info.getName());
						result = new PluginResult(PluginResult.Status.OK, jObj);
					}catch(JSONException jE){
						result = new PluginResult(PluginResult.Status.JSON_EXCEPTION);
					} finally{
						callbackContext.sendPluginResult(result);
						Log.d(TAG, "End onFinished");
					}
				}
			});
			Log.d(TAG, "End getInfo");
		} else{
